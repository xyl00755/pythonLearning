...
class webservice:
    ...
    def getTopicList_V2(self, token=None):
        url = '/common/pic/getTopicList_V2.json'
        data = {}
        self.wrapHttpBase.wspost(url, data, token)
        return self.wrapHttpBase.body

    # LB01.首页：获取品类图标（经销商红包，优惠券）
    def getNewTopicList(self,token=None):
        url='/common/pic/getNewTopicList.json'
        data={}
        self.wrapHttpBase.wspost(url,data,token)
        return self.wrapHttpBase.body

    #common_param_02.获取经销商最低配送金额
    def getDealerMinDeliveryAmt(self,dealerId=None,token=None):
        url='/common/param/getDealerMinDeliveryAmt.json'
        data={}
        data['dealerId']=dealerId
        return self.wrapHttpBase.wspost(url,data,token)

    # WQ07.获取经销商红包相关参数
    def getDealerCouponParam(self,dealerIdList=None,token=None):
        url='/common/param/getDealerCouponParam.json'
        data={}
        data['dealerIdList']=dealerIdList
        return self.wrapHttpBase.wspost(url,data,token)
    ...
    # merch_07.获取类目列表
    def getAllCategory(self, token=None):
        url = '/merch/getAllCategory.json'
        data={}
        self.wrapHttpBase.wspost(url, data, token)
        return self.wrapHttpBase.body
    ...
    def createOrderByShoppingcartNew(self,payWay=None, verfyCode=None, couponList=None, deliverAddress=None, invoice=None, sellerList=None, token=None):
        url = '/shoppingcart/createOrderByShoppingcartNew.json'
        data = {}
    ...
    #提交订单（pandora）
    def createOrderByShoppingcart_V3(self,payWay=None, verfyCode=None,orderTotalAmt=None ,couponList=None, deliverAddress=None, invoice=None, sellerList=None, token=None):
        url = '/shoppingcart/createOrderByShoppingcart_V3.json'
        data = {}
        if verfyCode is not None:
            data['verfyCode'] = verfyCode
        data['payWay'] = payWay
        data['orderTotalAmt']=orderTotalAmt
        if couponList is not None:
            data['couponList'] = couponList
        data['deliverAddress'] = deliverAddress
        if invoice is not None:
    ...
    def getSellerOrderDetail(self, orderPk, orderNo, token=None):
        url='/orders/getSellerOrderDetail.json'
        data = {}
        data['orderPk'] = orderPk
        data['orderNo'] = orderNo
        self.wrapHttpBase.wspost(url, data, token)
...
#0257.消息列表
    ...
    def getAllCouponCnt(self,token=None):
        url='/dealerCoupon/getAllCouponCnt.json'
        data={}
        return self.wrapHttpBase.wspost(url,data,token)

    # WQ04.提交订单页获取经销商可使用红包数量
    def getCouponCntWithDealer(self,dealerId=None,totalAmt=None,token=None):
        url='/dealerCoupon/getCouponCntWithDealer.json'
        data={}
        dealerParam=[]
        for j in range(0, len(dealerId)):
            dealerParam.append({"dealerId":dealerId[j], "totalAmt":totalAmt[j]})
        data['dealerParam']=dealerParam
        return self.wrapHttpBase.wspost(url,data,token)

    # LB04.商品详情：可领优惠券个数(与商品绑定)
    def getBenefitCntWithMerch(self,merchId=None,token=None):
        url='/dealerBenefit/getBenefitCntWithMerch.json'
        data={}
        data['merchId']=merchId
        return self.wrapHttpBase.wspost(url,data,token)

    # WQ01.获取我的经销商优惠券/红包 列表
    def getDealerCouponList(self,couponStatus=None,dealerCouponType=None,page=None,rows=None,token=None):
        url='/dealerCoupon/getCouponList.json'
        data={}
        data['couponStatus']=couponStatus
        data['dealerCouponType']=dealerCouponType
        data['page']=page
        data['rows']=rows
        return self.wrapHttpBase.wspost(url,data,token)

    # WQ02.获取我的经销商优惠劵/红包 数量
    def getCouponCnt(self,dealerCouponType=None,token=None):
        url='/dealerCoupon/getCouponCnt.json'
        data={}
        data['dealerCouponType']=dealerCouponType
        return self.wrapHttpBase.wspost(url,data,token)

    # WQ03.提交订单页获取商品优惠券
    def getCouponWithMerch(self,merchIdList=None,token=None):
        url='/dealerBenefit/getCouponWithMerch.json'
        data={}
        data['merchIdList']=merchIdList
        return self.wrapHttpBase.wspost(url,data,token)

    # WQ05.提交订单页获取经销商红包列表（包括：可用红包，不可用红包）
    def getCouponListWithDealer(self,dealerId=None,totalAmt=None,token=None):
        url='/dealerCoupon/getCouponListWithDealer.json'
        data={}
        data['dealerId']=dealerId
    ...
    def getActivityListWithMerch(self,merchId=None,token=None):
        url='/dealerBenefit/getActivityListWithMerch.json'
        data={}
        data['merchId']=merchId
        return self.wrapHttpBase.wspost(url,data,token)

    # LB03.领券中心、抢经销商红包：领取优惠券、抢经销商红包
    def achieveCoupon(self,activityId=None,token=None):
        url='/dealerCoupon/achieveCoupon.json'
        data={}
        data['activityId']=activityId
        return self.wrapHttpBase.wspost(url,data,token)

    #LB02.领券中心/抢经销商红包：可参与的活动列表(筛选条件)
    def getActivityList(self,dealerCouponType = None,categoryCode = None,dealerName = None,merchName = None,sortField = None,sortType = None,page = None,rows = None,token = None):
        url = '/dealerCoupon/getActivityList.json'
        data = {}
        data['dealerCouponType'] = dealerCouponType
