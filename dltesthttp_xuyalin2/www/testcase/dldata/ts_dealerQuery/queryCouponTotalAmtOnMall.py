...
class queryCouponTotalAmtOnMall(unittest.TestCase):
    ...
    #入参为空时查询到所有红包,
    def test_queryCouponTotalAmtOnMall_nullPara(self):
        queryResult=self.dlservice.queryCouponTotalAmtOnMall()
        self.assertEqual(queryResult['status'], -1)


    # 入参为空时查询到所有红包,
    def test_queryCouponTotalAmtOnMall_onlyRequiredPara(self):
        queryResult = self.dlservice.queryCouponTotalAmtOnMall(sellerId=self.UsedDealerCouponInfo.sellerId)
    ...
    #入参完整时查询到所有红包
    def test_queryCouponTotalAmtOnMall_AllPara(self):
        queryResult=self.dlservice.queryCouponTotalAmtOnMall(couponEntityId=self.UsedDealerCouponInfo.couponEntityId,
                                                              ...
                                                              couponUseStartTime= self.StartTime,couponUseEndTime= self.EndTime,
                                                              couponReceiveStartTime= self.StartTime,couponReceiveEndTime= self.EndTime
                                                              )
        self.assertEqual(queryResult['status'], 0)
        self.assertGreaterEqual(queryResult['data']['grantAmt'],0)
        self.assertGreaterEqual(queryResult['data']['discountAmt'], 0)
...
def suite():
    ...
    # suite.addTest(queryCouponTotalAmtOnMall("test_functions"))
    suite.addTest(queryCouponTotalAmtOnMall("test_queryCouponTotalAmtOnMall_AllPara"))
    suite.addTest(queryCouponTotalAmtOnMall("test_queryCouponTotalAmtOnMall_onlyRequiredPara"))
    suite.addTest(queryCouponTotalAmtOnMall("test_queryCouponTotalAmtOnMall_nullPara"))
    return suite
