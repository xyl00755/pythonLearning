...
class getSellerOrderDetail(unittest.TestCase):
    ...
    # S1.获取货到付款待发货卖家订单详情
    def test_getSellerOrderDetail_codWaitDeliver(self):
        orderDetail = self.wsDealMager.getSellerOrderDetail(orderPk=self.UserShop.orderCodWaitDeliver.paymentNo, orderNo=self.UserShop.orderCodWaitDeliver.orderNo)
    ...
    # S2.获取货到付款待收货卖家订单详情
    def test_getSellerOrderDetail_codWaitReceive(self):
        orderDetail = self.wsDealMager.getSellerOrderDetail(orderPk=self.UserShop.orderCodWaitReceive.paymentNo, orderNo=self.UserShop.orderCodWaitReceive['orderNo'])
        self.assertEqual(orderDetail.model['success'], '0')
        self.assertOrderDetail(orderDetail, self.UserShop.orderCodWaitReceive, self.UserShop, self.Merch1, '000001110')

    # S3.获取货到付款已取消卖家订单详情
    def test_getSellerOrderDetail_codCancel(self):
        orderDetail = self.wsDealMager.getSellerOrderDetail(orderPk=self.UserShop.orderCodCancel.paymentNo, orderNo=self.UserShop.orderCodCancel['orderNo'])
        self.assertEqual(orderDetail.model['success'], '0')
        self.assertOrderDetail(orderDetail, self.UserShop.orderCodCancel, self.UserShop, self.Merch1, '000001000')

    # S4.获取货到付款交易完成卖家订单详情
    def test_getSellerOrderDetail_codComplete(self):
        orderDetail = self.wsDealMager.getSellerOrderDetail(orderPk=self.UserShop.orderCodComplete.paymentNo, orderNo=self.UserShop.orderCodComplete['orderNo'])
        self.assertEqual(orderDetail.model['success'], '0')
        self.assertOrderDetail(orderDetail, self.UserShop.orderCodComplete, self.UserShop, self.Merch1, '000001000')

    # S5.获取终端店在线支付待付款买家订单详情
    def test_getSellerOrderDetail_onlineWaitPay(self):
        orderDetail = self.wsDealMager.getSellerOrderDetail(orderPk=self.UserShop.orderOnlineWaitPay.paymentNo, orderNo=self.UserShop.orderOnlineWaitPay['orderNo'])
        self.assertEqual(orderDetail.model['success'], '0')
        self.assertOrderDetail(orderDetail, self.UserShop.orderOnlineWaitPay, self.UserShop, self.Merch1, '000001100')
    ...
    def test_getSellerOrderDetail_onlineCancel(self):
        orderDetail = self.wsDealMager.getSellerOrderDetail(orderPk=self.UserShop.orderOnlienCancel.paymentNo, orderNo=self.UserShop.orderOnlienCancel['orderNo'])
        self.assertEqual(orderDetail.model['success'], '0')
        self.assertOrderDetail(orderDetail, self.UserShop.orderOnlienCancel, self.UserShop, self.Merch1, '000001000')

    # S11.获取货到付款订单拆单订单详情（交易中）
    def test_getSellerOrderDetail_codPayingSeparat(self):
        orderDetail = self.wsDealMager.getSellerOrderDetail(orderPk=self.UserShop.orderCodPayingSeparat.paymentNo, orderNo=self.UserShop.orderCodPayingSeparat['orderNo'])
        self.assertEqual(orderDetail.model['success'], '0')
    ...
    def test_getSellerOrderDetail_codCompleteSeparat(self):
        orderDetail = self.wsDealMager.getSellerOrderDetail(orderPk=self.UserShop.orderCodCompleteSeparat.paymentNo, orderNo=self.UserShop.orderCodCompleteSeparat['orderNo'])
        self.assertEqual(orderDetail.model['success'], '0')
    ...
    def test_getSellerOrderDetail_codWaitDeliverFullAdd(self):
        orderDetail = self.wsDealMager.getSellerOrderDetail(orderPk=self.UserShop.orderCodWaitDeliverFullAdd.paymentNo, orderNo=self.UserShop.orderCodWaitDeliverFullAdd['orderNo'])
        self.assertEqual(orderDetail.model['success'], '0')
    ...
    def test_getSellerOrderDetail_codWaitDeliverUnitDown(self):
        orderDetail = self.wsDealMager.getSellerOrderDetail(orderPk=self.UserShop.orderCodWaitDeliverUnitDown.paymentNo, orderNo=self.UserShop.orderCodWaitDeliverUnitDown['orderNo'])
        self.assertEqual(orderDetail.model['success'], '0')
    ...
    def test_getSellerOrderDetail_codWaitDeliverFullDown(self):
        orderDetail = self.wsDealMager.getSellerOrderDetail(orderPk=self.UserShop.orderCodWaitDeliverFullDown.paymentNo, orderNo=self.UserShop.orderCodWaitDeliverFullDown['orderNo'])
        self.assertEqual(orderDetail.model['success'], '0')
    ...
    def test_getSellerOrderDetail_orderNoUseCoupon(self):
        orderDetail = self.wsDealMager.getSellerOrderDetail(orderPk=self.UserShop2.orderNoUseCoupon.paymentNo, orderNo=self.UserShop2.orderNoUseCoupon['orderNo'])
        self.assertEqual(orderDetail.model['success'], '0')
        self.assertOrderDetail(orderDetail, self.UserShop2.orderNoUseCoupon, self.UserShop2, self.Merch4)



    def assertOrderDetail(self, rsq, order, buyer, merch, buttomList='010001110'):
        ...
        self.assertIsNotNone(rsq.model['orderDetail']['placeOrderTime'])
        self.assertEqual(rsq.model['orderDetail']['orderId'], orderInfo.id)
        ...
        if rsq.model['orderDetail']['payChannel'] is None or rsq.model['orderDetail']['payChannel'] == '':
            self.assertEqual(rsq.model['orderDetail']['payChannel'], '')
        else:
            self.assertEqual(rsq.model['orderDetail']['payChannel'], str(paymentOrder.pay_mechanism))
        ...
        # 现在反悔为buyerName，实际改为companyId
        self.assertEqual(rsq.model['orderDetail']['buyerId'], buyer.username)
...
def suite():
    ...
    suite.addTest(getSellerOrderDetail("test_getSellerOrderDetail_codPayingSeparat"))
    suite.addTest(getSellerOrderDetail("test_getSellerOrderDetail_codCompleteSeparat"))
    #suite.addTest(getSellerOrderDetail("test_getSellerOrderDetail_codWaitDeliverFullAdd"))
    # suite.addTest(getSellerOrderDetail("test_getSellerOrderDetail_codWaitDeliverUnitDown"))
    #suite.addTest(getSellerOrderDetail("test_getSellerOrderDetail_codWaitDeliverFullDown"))
    suite.addTest(getSellerOrderDetail("test_getSellerOrderDetail_orderNoUseCoupon"))
    return suite