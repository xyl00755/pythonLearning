...
"""
3.验证码校验
http://127.0.0.1:8080/mallws/shoppingcart/createOrderByShoppingcartNew.json
{
...
class createOrderByShoppingcartNew(unittest.TestCase):
    ...
    def test_createOrderByShoppingcartNew_one(self):
        ws = webservice()
        ...
        ws.checkSwitch()
        order = ws.createOrderByShoppingcartNew(payWay='2',invoice=invoice, deliverAddress=deliverAddress, sellerList=sellerList)
    ...
    def test_createOrderByShoppingcartNew_two(self):
        ws = webservice()
        ws.login(self.UserShop.username, self.UserShop.password)
        ws.addShoppingcar(merchId=self.Merch1.goodsId, merchCount='1', sellerId=self.Merch1.seller_store_id, sellerName=self.Merch1.sellerName)
    ...
    def test_createOrderByShoppingcartNew_twoDealer(self):
    ...
    # S4.无库存商品提交订单
    def test_createOrderByShoppingcartNew_outStock(self):
        ws = webservice()
        ws.login(self.UserShop.username, self.UserShop.password)
        ws.addShoppingcar(merchId=self.Merch1.goodsId, merchCount='1', sellerId=self.Merch1.seller_store_id, sellerName=self.Merch1.sellerName)
        shopcart = Shoppingcart.find_first('where user_id = ? and goods_id = ?', self.UserShop.userId, self.Merch1.goodsId)
        ...
        self.assertEqual(order.model['failedReasons']['invalidList'][0],self.Merch1.fullName)


    # S5.无售卖权商品提交订单
    def test_createOrderByShoppingcartNew_noSalerright(self):
        ws = webservice()
        ws.login(self.UserShop.username, self.UserShop.password)
        ws.addShoppingcar(merchId=self.Merch1.goodsId, merchCount='1', sellerId=self.Merch1.seller_store_id, sellerName=self.Merch1.sellerName)
        shopcart = Shoppingcart.find_first('where user_id = ? and goods_id = ?', self.UserShop.userId, self.Merch1.goodsId)
    ...
    def test_createOrderByShoppingcartNew_shortStock(self):
        ws = webservice()
        ws.login(self.UserShop.username, self.UserShop.password)
        ws.addShoppingcar(merchId=self.Merch1.goodsId, merchCount='2', sellerId=self.Merch1.seller_store_id, sellerName=self.Merch1.sellerName)
        shopcart = Shoppingcart.find_first('where user_id = ? and goods_id = ?', self.UserShop.userId, self.Merch1.goodsId)
        # 设置库存为空
        update('update dlmerchandise.dl_goods set on_hand_inventory = ? where goods_id = ?', 1, self.Merch1.goodsId)
        ...
        order = ws.createOrderByShoppingcartNew(payWay='2',invoice=invoice, deliverAddress=deliverAddress, sellerList=sellerList)
        self.assertEqual(order.model['success'], '2')
        self.assertEqual(order.model['failedReasons']['noOnHandInventoryList'][0],self.Merch1.fullName)
    ...
    # S13、提交一个在线支付订单
    def test_createOrderByShoppingcartNew_online(self):
        ws = webservice()
        ws.login(self.UserShop.username, self.UserShop.password)
        ws.addShoppingcar(merchId=self.Merch1.goodsId, merchCount='1', sellerId=self.Merch1.seller_store_id, sellerName=self.Merch1.sellerName)
        shopcart = Shoppingcart.find_first('where user_id = ? and goods_id = ?', self.UserShop.userId, self.Merch1.goodsId)
        ...
        sellerList.append({"sellerId":self.Merch1.shopcartSellerId,"sellerName":self.Merch1.sellerName,"isYijipayAccount":self.Merch1.isYijipayAccount,"codFlag":self.Merch1.codFlag,
                           "supportVatInvoice":self.Merch1.supportVatInvoice,"comment":"createOrderByShoppingcartNew comment.","merchList":
                               [{"id":shopcart.id,"merchId":self.Merch1.goodsId,"merchBarCode":self.Merch1.productBarCode}]})
        ws.checkSwitch()
        order = ws.createOrderByShoppingcartNew(payWay='1',invoice=invoice, deliverAddress=deliverAddress, sellerList=sellerList)
        self.assertEqual(order.model['success'], '0')

        # 校验订单号和交易号是否匹配
        orderInfoDb = select_one('select * from dlorder.dl_order_orderinfo where pay_no = ?', order.model['createOrderInfoModel']['onlinePaymentModelList'][0]['paymentNo'])
        # 支付方式为在线支付
        self.assertEqual(orderInfoDb.pay_type, '1')
        self.assertEqual(orderInfoDb.order_no, order.model['createOrderInfoModel']['onlinePaymentModelList'][0]['orderNo'])
        self.assertEqual(orderInfoDb.order_amount, int(order.model['createOrderInfoModel']['onlinePaymentModelList'][0]['totalPrice']))

        # 校验发票是否是提交的发票
        orderInvoiceDb = select_one('select * from dlorder.dl_order_orderinvoice where pay_no = ?', order.model['createOrderInfoModel']['onlinePaymentModelList'][0]['paymentNo'])
        self.assertEqual(orderInvoiceDb.invoice_header, invoice['invoiceHeader'])

        # 校验收货地址是否是提交的收货地址
        orderDetailDb = select_one('select * from dlorder.dl_order_orderdetail where order_no = ?', order.model['createOrderInfoModel']['onlinePaymentModelList'][0]['orderNo'])
        self.assertEqual(orderDetailDb.buyer_id, self.UserShop.companyId)
        self.assertEqual(orderDetailDb.receive_person, deliverAddress['deliverPerson'])
        self.assertEqual(orderDetailDb.receive_tel, deliverAddress['deliverMobile'])
        self.assertEqual(orderDetailDb.receive_address, deliverAddress['deliverAddress'])
        ...
        # 校验商品是否是提交的商品
        orderItemDb = select_one('select * from dlorder.dl_order_orderitem where order_no = ?', order.model['createOrderInfoModel']['onlinePaymentModelList'][0]['orderNo'])
        self.assertEqual(orderItemDb.merchandise_id, self.Merch1.goodsId)
        self.assertEqual(orderItemDb.num, 1)
...
def suite():
    ...
    suite.addTest(createOrderByShoppingcartNew("test_createOrderByShoppingcartNew_one"))
    suite.addTest(createOrderByShoppingcartNew("test_createOrderByShoppingcartNew_two"))
    suite.addTest(createOrderByShoppingcartNew("test_createOrderByShoppingcartNew_twoDealer"))
    suite.addTest(createOrderByShoppingcartNew("test_createOrderByShoppingcartNew_outStock"))
    #suite.addTest(createOrderByShoppingcartNew("test_createOrderByShoppingcartNew_noSalerright"))
    suite.addTest(createOrderByShoppingcartNew("test_createOrderByShoppingcartNew_shortStock"))

    suite.addTest(createOrderByShoppingcartNew("test_createOrderByShoppingcartNew_online"))
    return suite