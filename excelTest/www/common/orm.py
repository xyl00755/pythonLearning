...
class DatetimeField(Field):
    ...
    """
    def __init__(self):
        super(DatetimeField, self).__init__(ddl='datetime')
...
class Model(dict):
    ...
    u'orm@db.org'
    >>> r = g.delete()
    >>> len(database.select('select * from user where id=10190'))
    0
    >>> import json
    >>> print User().__sql__()
    ...
    def find_all(cls, *args):
        ...
        查询所有字段， 将结果以一个列表返回
        """
        L = database.select('select * from `%s`' % cls.__table__)
    ...
    def count_all(cls):
        ...
        """
        return database.select('select count(`%s`) from `%s`' % (cls.__primay_key__.name, cls.__table__))
    ...
    def count_by(cls, where, *args):
        ...
        """
        return database.select_int('select count(`%s`) from `%s` %s' % (cls.__primary_key__.name, cls.__table__, where), *args)
    ...
    def insert(self):
        ...
        database.insert('%s' % self.__table__, **params)
        return self